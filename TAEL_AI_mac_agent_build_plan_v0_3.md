...
**Version:** v0.3  
**Status:** repo-ready implementation freeze for v1 prototype  
**Date:** 2026-04-25  
...
- v1 prototype deployment target is **macOS 14.0+**.
- Preferred screenshot path is `SCScreenshotManager.captureImage(contentFilter:configuration:)`.
- Do not use `captureImage(in:)` unless the deployment target intentionally moves to macOS 15.2+.
- Week 1 is only the first heartbeat: hotkey -> permission gate -> screenshot -> non-activating HUD.
- `PermissionsGate` is an architectural boundary, not a convenience helper.
- No protected macOS API call may bypass `PermissionsGate`.
- Build the first three skills as hardcoded Swift structs before writing any YAML.
...
- Lock stable bundle ID, macOS 14.0+ target, signing team, and Debug signing identity before TCC work.
- Week 1 screenshot target is the display containing the cursor, with fallback to main display. Do not call Week 1 capture "focused-window" capture.
...
| Layer | Choice |
|---|---|
| Platform | macOS first, v1 deployment target macOS 14.0+ |
| Language | Swift |
| UI | SwiftUI with AppKit where needed |
| App shape | Menubar app |
| HUD | `NSPanel`, non-activating |
| Hotkey | KeyboardShortcuts package |
| Screenshot | ScreenCaptureKit / `SCScreenshotManager.captureImage(contentFilter:configuration:)` |
...
- Xcode project,
- stable app bundle identifier,
- macOS 14.0+ deployment target,
- signing team and Debug signing identity,
- Developer ID signing plan,
- app icon placeholder,
- Week 1-only folder structure,
...
- Week 1 screenshot target: display containing cursor, fallback to main display,
- `SCScreenshotManager.captureImage(contentFilter:configuration:)` screenshot,
- `SCShareableContent`, `SCContentFilter`, and `SCStreamConfiguration` setup,
- explicit screenshot width and height based on chosen display and backing scale,
...
- permission retry state works,
- screenshot of the display containing the cursor appears in HUD,
- screenshot is not persisted to disk,
- HUD does not crash in full-screen mode,
...
For command preview, planner output must use structured executable + argv commands. The HUD may render these commands as readable shell lines, but the executor must not receive a raw composite shell string.

```json
{
  "skill_id": "dev.git.commit_preview",
  "confidence": 0.91,
  ...
  "actions": [
    {
      "kind": "shell.sequence.preview",
      "cwd": "/Users/ozzy/Projects/my-repo",
      "commands": [
        {
          "executable": "/usr/bin/git",
          "argv": ["git", "add", "tests/login.spec.ts"],
          "risk": "medium",
          "requires_confirmation": true
        },
        {
          "executable": "/usr/bin/git",
          "argv": ["git", "commit", "-m", "fix: update login test selector"],
          "risk": "medium",
          "requires_confirmation": true
        }
      ]
...
```text
v1 prototype deployment target: macOS 14.0+
Preferred screenshot API: SCScreenshotManager.captureImage(contentFilter:configuration:)
Do not use captureImage(in:) unless macOS 15.2+ becomes acceptable.
```

### 23.2 Stable bundle, signing, and TCC setup

Before permission or screenshot work, lock:

```text
Bundle ID: ai.tael.macagent
Deployment target: macOS 14.0+
Debug signing: Apple Development identity with stable team
Release signing later: Developer ID + notarization
```

Do not use ad-hoc signing for regular development builds. TCC permissions are keyed to bundle identity and signing requirements, and unstable signing makes permission debugging noisy.

Week 1 implements Screen Recording permission only. Accessibility and Microphone may exist as enum placeholders, but they should not request real permissions until their milestones.
...
Do not call Week 1 capture focused-window capture. Focused-window context starts when AX work begins.

### 23.3 Week 1 hard boundary
...
### 23.4 Day 5 heartbeat definition of done
...
### 23.5 First 13 implementation tickets

| Ticket | Title | Acceptance |
|---:|---|---|
| 0 | Lock bundle ID, macOS 14.0+ target, signing team, and Debug signing identity | App has stable identity before any TCC-protected work. |
| 1 | Create native macOS menubar shell | App launches as menubar utility and can quit from menu. |
| 2 | Add `ProtectedAPICallPolicy.md` | Documents screen, AX, mic, Apple Events, subprocess, clipboard policies. |
| 3 | Add `PermissionKind` and `PermissionStatus` | Types exist before protected services. Accessibility and Microphone can be placeholders. |
| 4 | Implement `PermissionsChecker` v0 for Screen Recording only | Can report Screen Recording status using the macOS ScreenCapture preflight path. |
| 5 | Implement `PermissionsGate` v0 | Protected operation cannot run without passing through tokenized gate. Missing permission shows placeholder gate and retry path. |
| 6 | Add KeyboardShortcuts package | Temporary hotkey or user-configurable shortcut triggers a callback. |
| 7 | Add `HUDPanelController` placeholder | Shows a non-activating `NSPanel` with placeholder content and does not steal focus. |
| 8 | Add `ScreenCaptureService` | Captures display-containing-cursor screenshot using `SCShareableContent`, `SCContentFilter`, `SCStreamConfiguration`, and `SCScreenshotManager.captureImage(contentFilter:configuration:)`. |
| 9 | Wire hotkey -> screen gate -> screenshot capture | Missing permission shows gate state instead of crashing. Granted permission captures screenshot. |
| 10 | Render screenshot in HUD | Screenshot appears in panel and is not persisted to disk. |
| 11 | Add local invocation log | Logs hotkey timestamp, permission status, screenshot timing, target display metadata, and error state. |
| 12 | Add manual test checklist | Terminal, VS Code/Cursor, full-screen, denied permission, revoked permission, display-containing-cursor, no-persistence check. |

### 23.6 `PermissionsGate` as tokenized code boundary

`PermissionsGate` wraps protected operations. Protected services should not expose public methods that bypass the gate.
...
Protected services accept the grant and assert the expected kind:
...
### 23.6 Speech dependency boundary

WhisperKit must be isolated behind `TranscriptionService`.

```swift
protocol TranscriptionService {
    func transcribe(_ audio: RecordedAudioClip) async throws -> Transcript
}

final class WhisperKitTranscriber: TranscriptionService {
    // The only module allowed to import WhisperKit.
}
```

Do not let the main app import Argmax packages broadly. Pin the chosen package source in `Package.resolved` when speech work starts.

### 23.7 Structured executable action format

Do not execute composite shell strings. The HUD may display readable Bash, but the executor receives structured executable + argv values.
...
```json
{
  ...
  "commands": [
    ...
    {
      "executable": "/usr/bin/git",
      "argv": ["git", "commit", "-m", "fix: update login selector in Playwright test"],
      "category": "git_commit",
      "risk": "medium",
      "requires_confirmation": true
    }
...
Layer 2: action category policy.

```text
read_only      -> allowed
clipboard      -> allowed unless external-send implied
file_write     -> confirmation
file_delete    -> confirmation + stronger warning
git_add        -> confirmation
git_commit     -> confirmation
git_push       -> blocked in v1
network        -> confirmation or block
install        -> confirmation + dependency warning
secrets        -> blocked
external_post  -> blocked in v1
```

Layer 3: executor capability boundary.

```swift
enum ExecutorCapability {
    case clipboardWrite
    case readOnlySubprocess
    case fileWrite
    case gitCommit
    case appleScript
    case keyboardEvent
...
```text
planned action -> schema validation -> safety policy -> permission gate -> executor capability -> user approval -> run
```

### 23.9 Skill naming correction

Use `dev.git.commit_preview` for Skill 3. The user thinks “commit this.” The system decides whether staging is part of the preview.
...
```text
SCShareableContent -> SCContentFilter -> SCStreamConfiguration -> SCScreenshotManager.captureImage(contentFilter:configuration:)
```

Rules:

- Do not use `captureImage(in:)` in v1 unless the deployment target moves to macOS 15.2+.
- Choose the display containing the cursor. If that cannot be resolved, use the main display.
- Set `SCStreamConfiguration.width` and `SCStreamConfiguration.height` intentionally based on selected display dimensions and backing scale.
- Exclude the app's own HUD from capture when possible.
- Do not persist screenshots to disk in Week 1.
- Log screenshot timing, selected display metadata, permission status, and error state.

### 23.11 Week 1 HUD panel defaults

Use a real `NSPanel`, not a plain `NSWindow`.

Recommended Week 1 defaults:
...
Do not call `NSApp.activate()` in the heartbeat path. The HUD should appear without stealing focus from Terminal, VS Code, or Cursor.

### 23.12 Day 1 repo files and folders

Create only the Week 1 structure. Do not add future folders until the milestone needs them.
...
Do not add these folders on day 1:
...
### 23.13 First commit and first PR scope
...
```text
- Buildable Xcode macOS app project
- macOS 14.0+ deployment target
- Stable bundle ID
- Debug signing identity/team selected or documented
- Menubar app shell
- Quit menu item
- README with Week 1 heartbeat
- ProtectedAPICallPolicy.md
- PermissionNotes.md
- ManualTestChecklist.md
- Initial Week 1 folder structure
- .gitignore
```

First PR proves only:

```text
App launches as native macOS menubar utility.
App has stable bundle/signing setup.
App can quit cleanly.
Repo contains Week 1 policy/docs.
```
...
product naming
landing page work
real microphone permission checks
...
## 25. One final instruction for the build
