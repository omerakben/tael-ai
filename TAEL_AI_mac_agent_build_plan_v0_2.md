...
**Version:** v0.2  
**Status:** implementation freeze for v1 prototype  
**Date:** 2026-04-25  
...
### 23.3 Day 5 heartbeat definition of done
...
### 23.4 First 12 implementation tickets
...
| 3 | Add `PermissionKind` and `PermissionStatus` | Types exist before protected services. |
| 4 | Implement `PermissionsChecker` v0 | Can report screen recording, accessibility, and microphone status. |
| 5 | Implement `PermissionsGate` v0 | Protected operation cannot run without passing through gate. |
| 6 | Add KeyboardShortcuts package | Temporary hotkey or user-configurable shortcut triggers a callback. |
| 7 | Add `HUDPanelController` | Shows a non-activating `NSPanel` with placeholder content. |
| 8 | Add `ScreenCaptureService` | Captures a screenshot using the macOS 14-compatible ScreenCaptureKit path. |
| 9 | Wire hotkey to screen gate | Missing permission shows gate state instead of crashing. |
| 10 | Render screenshot in HUD | Screenshot appears in panel. |
| 11 | Add local invocation log | Logs hotkey timestamp, permission status, screenshot timing, and error state. |
| 12 | Add manual test checklist | Terminal, VS Code/Cursor, full-screen, denied permission, revoked permission. |

### 23.5 `PermissionsGate` as code boundary
...
```swift
let image = try await permissionsGate.withPermission(.screenRecording) {
    try await screenCaptureService.captureFocusedDisplayScreenshot()
}
```
